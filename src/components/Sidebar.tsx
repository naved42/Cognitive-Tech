...
export const Sidebar = ({ collapsed, setCollapsed, activeTab, setActiveTab }: SidebarProps) => {
  ...
  return (
    <aside 
      className={cn(
        "h-screen bg-[#F9FAFB] dark:bg-[#080808] border-r border-[#E5E7EB] dark:border-zinc-800 transition-all duration-300 flex flex-col z-50 relative",
    ...
    >
      ...
      <div className="p-4">
        {!collapsed ? (
          <button className="w-full flex items-center justify-between px-3 py-2 rounded-lg bg-white dark:bg-zinc-900 border border-[#E5E7EB] dark:border-zinc-800 shadow-sm hover:bg-gray-50 dark:hover:bg-zinc-800 transition-all group">
          ...
          <Tooltip>
            <TooltipTrigger>
              <div className="w-8 h-8 mx-auto rounded-md bg-indigo-600 flex items-center justify-center text-white text-[10px] font-bold shadow-lg shadow-indigo-500/20 cursor-pointer">W</div>
            </TooltipTrigger>
      ...
      <div className="px-4 py-2">
        {!collapsed ? (
          ...
          <Tooltip>
            <TooltipTrigger>
              <Button size="icon" variant="ghost" onClick={() => setActiveTab('analysis')} className="mx-auto rounded-full border border-gray-200 dark:border-zinc-800 shadow-sm">
        ...
        {navSections.map((section, idx) => (
          <div key={idx} className="space-y-1">
            {!collapsed && section.title && (
              <p className="px-3 text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2 opacity-70">
                {section.title}
              </p>
            )}
                ...
                return (
                  <Tooltip key={item.id} disabled={!collapsed}>
                    <TooltipTrigger>
                      ...
                      >
                        <item.icon className={cn("w-4 h-4 shrink-0", isActive ? "text-indigo-600 dark:text-indigo-400" : "text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white")} />
                        {!collapsed && (
                          <div className="flex items-center justify-between flex-1">
                            <span>{item.label}</span>
                            {item.isPlaceholder && (
                              <span className="text-[8px] bg-gray-100 dark:bg-zinc-800 px-1.5 py-0.5 rounded text-gray-400 font-bold uppercase tracking-tighter">Soon</span>
                            )}
                          </div>
                        )}
                      </button>
